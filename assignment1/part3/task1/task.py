...
# set initial ranks of all keys formed above to 1
ranks = edges.mapValues(lambda edges: 1.0)

# run the pagerank algorithm for 10 iterations
for i in range(1, 11):
    # (A, B) when joined with (A, C) gives (A, (B, C))
    new_ranks = edges.join(ranks) \
                    .flatMap(lambda edge_rank_info: calc_contribution(edge_rank_info[1][1], edge_rank_info[1][0])) \
                    .reduceByKey(lambda c1, c2 : c1+c2)
    ranks = new_ranks.mapValues(lambda rank: rank*0.85 + 0.15)
